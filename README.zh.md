...
- **QQ 是第二个平台，不是一个分支** —— 同一座桥现在同时支持微信与 QQ。平台接缝
  （`src/platform/index.ts`）是唯一知道"这条会话属于哪条渠道"的地方，两个平台各自拥有
  **自己的**白名单、记忆文件、问题台账与会话命名空间。两条渠道的对话永远不共用上下文，
  也不会互相镜像转发。
- **一个控制台管两个平台** —— 管理台可以在同一个网址下服务多个 profile（`--profiles wechat,qq`）
  并切换。控制台读的每一个路径都由**平台 id** 推出，所以"名叫 `web`、实际服务微信"的 profile
  能找到 `wechat-memory/MEMORY.md` 与 `wechat-*` 会话，而不是对着一屋子数据渲染空页面。
- **控制台再也走不出自己的目录** —— `?platform=` 以前不经校验就进了路径拼接，构造一个名字
  就能读写 `profiles/` 之外的 `cordis.patch.yml`。现在收口到一处校验，并有回归测试钉住"必须拒绝"。
- **改前端立刻生效** —— `admin/index.html` 每次请求都读盘，改完刷新即可，不用重启控制台。
- **保存不再把 profile 写坏** —— YAML 标量按字段 `kind` 决定是否加引号：QQ AppID 这类
  `"1234567890"` 会保持字符串，不会被写成数字、导致下次启动被 schema 拒绝。

上一版的重头戏（网关传输自愈、长轮询窗口封顶、长回合进度提前）见
[v4.1 发行说明](releases/v4.1-release-notes.md)。逐条变更：[CHANGELOG.md](CHANGELOG.md) · 更早的版本：[releases/](releases/)
...
<details open>
<summary><b>v5.0</b> —— QQ 接入为第二平台 · 一个控制台管两边 · 控制台路径护栏</summary>

- **双平台，一座桥**：微信与 QQ，各自拥有白名单、记忆文件、问题台账与会话命名空间。按平台的
  接缝决定一个回合属于哪条渠道，答复永远回到它来的那条渠道。
- **多平台管理台**：同一个网址服务多个 profile 并可切换；控制台读的每个文件都按平台 id 定位，
  会话、记忆、台账都能显示出来，而不是渲染空页面。
- **安全**：控制台的 `?platform=` 在变成路径之前先校验 —— 未知名字与穿越写法一律 4xx，并有
  回归测试钉住；profile 名单也在启动时校验。
- **修复**：字符串型数字值（如 QQ AppID）不再被写成裸 YAML 数字，那个问题会让保存过的
  profile 下次启动直接失败。
- **修复**：问题台账与控制台的命令队列改为按平台 id 定位，不再按 profile 名。
- 294 条离线单元测试。

见 [`releases/v5.0-release-notes.md`](releases/v5.0-release-notes.md)。
