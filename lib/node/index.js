...
export function apply(ctx, config) {
    ...
    const unregisterSendFile = ctx.tools.register(defineTool({
        ...
        description: 'Send a local file (document, archive, pdf, mp3, …) to the current WeChat peer through the ' +
        ...
        execute: async (args) => {
            const path = typeof args.path === 'string' ? args.path.trim() : '';
            if (!path)
                throw new Error('wechat_send_file: path is required');
            const peer = node.peerId;
            if (!peer) {
                throw new Error('wechat_send_file: no WeChat peer yet — send the bot a WeChat message first ' +
                    'so the bridge knows who to reply to');
            }
            const result = await node.ctx.wechat.sendFile(peer, path);
            if (!result.success)
                throw new Error(`wechat_send_file: ${result.error}`);
    ...
    const unregisterSendVideo = ctx.tools.register(defineTool({
        ...
        description: 'Send a local video file (mp4/mov/webm/…) to the current WeChat peer through the chatnode-wechat ' +
        ...
        execute: async (args) => {
            const path = typeof args.path === 'string' ? args.path.trim() : '';
            if (!path)
                throw new Error('wechat_send_video: path is required');
            const peer = node.peerId;
            if (!peer) {
                throw new Error('wechat_send_video: no WeChat peer yet — send the bot a WeChat message first ' +
                    'so the bridge knows who to reply to');
            }
            const result = await node.ctx.wechat.sendFile(peer, path);
            if (!result.success)
                throw new Error(`wechat_send_video: ${result.error}`);
    ...
    const unregisterGenerateImage = ctx.tools.register(defineTool({
        name: 'generate_image',
        description: 'Generate an image from a text prompt (SiliconFlow text-to-image) and send it to the current WeChat peer. ' +
        ...
        execute: async (args) => {
            ...
            const peer = node.peerId;
            if (!peer)
                throw new Error('generate_image: no WeChat peer yet — the user must message the bot first');
            ...
            const result = await generateImage({ apiKey, model: config.imageGenModel, outDir }, prompt);
    ...
    const unregisterSpeak = ctx.tools.register(defineTool({
        name: 'speak',
        description: 'Speak the given text to the user: it is converted to speech with the cloned voice (e.g. Shiroko) ' +
        ...
        execute: async (args) => {
            ...
            const peer = node.peerId;
            if (!peer)
                throw new Error('speak: no WeChat peer yet — the user must message the bot first');
    ...
    const unregisterSetReminder = ctx.tools.register(defineTool({
        ...
        execute: async (args) => {
            ...
            const peer = node.peerId;
            if (!peer)
                throw new Error('set_reminder: no WeChat peer yet — the user must message the bot first');
