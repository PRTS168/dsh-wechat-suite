...
export function readContextUsage(sessionId) {
    try {
...
export function readTurnCount(sessionId) {
    try {
        const file = join(dshHome(), 'storages', 'session_projcache', 'sessions', `${sessionId}.json`);
        const doc = JSON.parse(readFileSync(file, 'utf8'));
        const stats = doc.record?.rows?.sessionStats?.val;
 ...
 * does not have.
 */
export function readCompactionPolicy(presetId) {
    try {
        ...
        if (!threshold && !retain)
            return { ...DEFAULT_COMPACTION, source: 'default' };
        const parsed = {
...
export function formatContextReport(input) {
    const lines = [`📊 上下文 · 会话 ${input.sessionId}${input.turns === undefined ? '' : ` · ${input.turns} 轮`}`];
    ...
    lines.push(usage.contextWindow === undefined
        ? `已用约 ${formatTokens(used)} token`
        : `已用 ${formatTokens(used)} / ${formatTokens(usage.contextWindow)}（${percent(used, usage.contextWindow)}）`);
    const parts = [];
    if (usage.messageTokens !== undefined)
        parts.push(`对话 ${formatTokens(usage.messageTokens)}`);
    if (usage.toolsTokens !== undefined)
        parts.push(`工具 ${formatTokens(usage.toolsTokens)}`);
    if (usage.systemTokens !== undefined)
        parts.push(`系统 ${formatTokens(usage.systemTokens)}`);
    ...
    if (policy && usage.contextWindow !== undefined) {
        ...
        lines.push(`压缩：到 ${formatTokens(trigger)} 才自动触发（${Math.round(policy.thresholdRatio * 100)}%），最近约 ${formatTokens(keep)} token 的对话原样保留`);
        if (policy.source === 'default')
            lines.push('　（这是宿主默认值，preset 里没写死）');
    }
