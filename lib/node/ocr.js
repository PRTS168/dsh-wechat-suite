 ...
 */
export async function ocrImage(config, bytes) {
    if (!config.apiKey)
        throw new Error('ocrImage: no apiKey configured');
    if (bytes.length === 0)
        throw new Error('ocrImage: empty image');
    if (bytes.length > MAX_IMAGE_BYTES)
        throw new Error(`ocrImage: image too large (${bytes.length} bytes)`);
