...
import { platformLabel, USER_MESSAGE_OPEN_ANY } from "../platform/index.js";
// ---------------------------------------------------------------------------
 ...
 */
export function resolveOutboundTarget(node) {
 ...
 */
export function scheduleDigests(intervalSec, tick, earlyMs = FIRST_DIGEST_DELAY_MS) {
...
export function attachSessionOutbound(node) {
    ...
    const onEvent = (session, event) => {
        ...
        // chat peer.
        if (!node.isOwnSessionId(session.id))
            return;
