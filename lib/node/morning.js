...
import { chatService, platformNamespace } from "../platform/index.js";
...
export class MorningService {
    ...
    constructor(ctx, opts) {
        ...
        this.file = opts.file ?? join(process.env.DSH_HOME ?? join(homedir(), '.dsh'), `${platformNamespace(this.platform)}morning.json`);
    ...
    async update(patch) {
        ...
        this.arm();
        return { config: this.getConfig(), saved };
    }
    ...
    async load() {
        try {
            ...
            this.ctx.logger?.warn?.('[dsh-chatnode-wechat] morning config unreadable (%s): %s', this.file, error instanceof Error ? error.message : String(error));
        }
    }
    /**
    ...
    async save() {
        ...
        try {
            ...
            this.onProblem?.('morning/persist', error);
            return false;
        }
    }
    ...
    arm() {
        ...
        // A clock that cannot be read (or a NaN delay) makes setTimeout fire
        // immediately, and the callback re-arms: that is a busy loop hammering the
        // weather API with no error anywhere. Refuse to arm instead, and say why.
        if (!Number.isFinite(armDelay) || armDelay < 0) {
            this.ctx.logger?.warn?.('[dsh-chatnode-wechat] morning greeting not armed: unusable delay %s (time=%s, now=%s)', String(armDelay), this.config.time, String(now));
            this.onProblem?.('morning/clock', new Error(`早安定时器无法计算下一次触发时间（delay=${String(armDelay)}）`));
            return;
        }
