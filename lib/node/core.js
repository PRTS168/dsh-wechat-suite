...
import { chatService, isPlatformId, platformEvents, } from "../platform/index.js";
...
export class WechatConversationNode {
    ...
    constructor(ctx, config) {
        ...
        const disposer = this.ctx.on(this.events.message, (message) => {
...
export function attachGatewayObservability(node) {
    const disposers = [];
    disposers.push(node.ctx.on(node.events.status, ((status) => {
    ...
    disposers.push(node.ctx.on(node.events.error, ((error) => {
    ...
    disposers.push(node.ctx.on(node.events.fatal, ((error) => {
