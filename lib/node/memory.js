...
import { platformNamespace } from "../platform/index.js";
...
export function defaultMemoryFile(platform = 'wechat') {
...
export function extractUtterances(events) {
    ...
    // channel's sessions.
    const envelope = new RegExp(`(?:${USER_MESSAGE_OPEN_ANY.join('|')})\\n([\\s\\S]*?)\\n<<<(?:${USER_MESSAGE_CLOSE_ANY.join('|')})｜发送于\\s*([0-9-]+ [0-9:]+)>>>`, 'g');
...
export class MemoryService {
    ...
    async ask(route, session, prompt) {
 ...
 * legitimate value.
 */
export async function askSessionModel(ctx, route, sessionId, prompt, maxTokens = 1024) {
