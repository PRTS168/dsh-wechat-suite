 ...
 */
import { USER_MESSAGE_CLOSE_ANY, USER_MESSAGE_OPEN_ANY } from "../platform/index.js";
 ...
 */
export function parseContextPolicy(raw, onProblem) {
    if (!raw || !raw.trim())
        return { ...DEFAULT_POLICY };
    try {
        const parsed = JSON.parse(raw);
        const scheme = POLICY_SCHEMES.includes(String(parsed.scheme))
            ? parsed.scheme
            : DEFAULT_POLICY.scheme;
        if (parsed.scheme !== undefined && String(parsed.scheme) !== scheme) {
            onProblem?.('context/policy', new Error(`未知的上下文方案「${String(parsed.scheme)}」，已按 ${scheme} 处理`), `raw=${raw.slice(0, 200)}`);
        }
        const policy = { ...DEFAULT_POLICY, ...parsed, scheme };
        ...
        if (policy.turns !== undefined && (!Number.isFinite(policy.turns) || policy.turns < 1))
            policy.turns = DEFAULT_POLICY.turns;
        ...
        if (policy.idleHours !== undefined && (!Number.isFinite(policy.idleHours) || policy.idleHours <= 0))
            policy.idleHours = DEFAULT_POLICY.idleHours;
 ...
 */
export function rotationReason(policy, signals) {
    if (policy.scheme === 'manual')
        return undefined;
    if (policy.idleOnly !== false && signals.turnOpen)
        return undefined;
...
function stripEnvelope(text) {
    // 两种平台的围栏都要剥：合并会话里两种都在，而历史里只有旧的微信写法。
    const open = new RegExp(`(?:${USER_MESSAGE_OPEN_ANY.join('|')})\\n?`, 'g');
    const close = new RegExp(`\\n?<<<(?:${USER_MESSAGE_CLOSE_ANY.join('|')})｜发送于 [^>]*>>>`, 'g');
    return text
        .replace(open, '')
        .replace(close, '')
 ...
 */
export function signalsFromEvents(events, now = new Date()) {
    let turns = 0;
    let turnOpen = false;
    let contextChars = 0;
    let lastTime = 0;
 ...
 */
export function buildHandoff(lines, reason, options = {}) {
