...
import { ITEM_TEXT, ITEM_VOICE, ITEM_IMAGE, ITEM_VIDEO } from "../gateway/types.js";
...
import { isPlatformId, platformLabel, userFence } from "../platform/index.js";
...
export { USER_MESSAGE_CLOSE_ANY, USER_MESSAGE_OPEN_ANY } from "../platform/index.js";
/**
 ...
 * message" never is).
 */
export function wrapUserMessage(content, date = new Date(), platform = 'wechat') {
 ...
 */
function wrapForModel(node, content, platform) {
 ...
 * 这里只是让它**不用白跑一趟**。
 */
function channelNotice(node, platform) {
...
function isAllowedOn(node, platform, sender) {
    if (typeof node.isAllowedFor === 'function')
        return node.isAllowedFor(platform, sender);
    return node.isAllowed(sender);
}
/** The gateway the message arrived on — downloads go through **that** one. */
function chatOn(node, platform) {
    if (typeof node.chatFor === 'function')
        return node.chatFor(platform);
    return node.chat;
}
/**
 ...
 */
function gatewayAccountOn(node, platform) {
    if (typeof node.gatewayAccountIdFor === 'function')
        return node.gatewayAccountIdFor(platform);
    return node.gatewayAccountId;
}
/** Remember where this message came from (see `core.noteInboundTarget`). */
function noteInbound(node, platform, sender) {
    if (typeof node.noteInboundTarget === 'function')
        node.noteInboundTarget(platform, sender);
    else
        node.peerId = sender;
}
/**
 ...
 */
function submitTurn(node, platform, sender, submit) {
    const tracked = typeof node.beginTurnTarget === 'function';
    if (tracked)
        node.beginTurnTarget(platform, sender);
    try {
        submit();
    }
    catch (error) {
...
function inboundMediaDir(node, platform) {
 ...
 */
async function handleInboundImage(node, platform, sender, message) {
 ...
 */
async function handleInboundVoice(node, platform, sender, message) {
    const items = Array.isArray(message.item_list) ? message.item_list : [];
    ...
    try {
        ...
        { apiKey, model: node.config.sttModel, baseUrl: node.config.ocrBaseUrl }, bytes);
    }
    catch (error) {
 ...
 */
async function handleInboundFile(node, platform, sender, message, kind) {
 ...
 */
export async function handleInbound(node, message, platform = node.platform) {
    const source = isPlatformId(platform) ? platform : 'wechat';
    ...
    if (!isAllowedOn(node, source, sender)) {
        ...
        node.problems.report('inbound/allowlist', new Error(`忽略了一条非白名单消息：sender=${sender}。若这是你自己，把这个 id 加进 ${where} 即可。`), { notify: false, detail: `platform=${source}` });
        node.ctx.logger?.info?.('[dsh-chatnode-wechat] ignoring non-allowlisted %s message from %s (never fed to the model)', source, sender);
        return;
    }
    if (isGroupMessage(message, gatewayAccountOn(node, source))) {
        node.ctx.logger?.info?.('[dsh-chatnode-wechat] ignoring %s group message from %s (MVP: no group support)', source, sender);
        return;
    }
