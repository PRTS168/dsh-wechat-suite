...
function findPersonaBlock(lines) {
    let rowStart = -1;
    let rowIndent = 0;
    for (let i = 0; i < lines.length; i++) {
        const m = /^(\s*)-\s*id:\s*(\S+)\s*$/.exec(lines[i]);
        if (m && /persona/i.test(m[2])) {
            rowStart = i;
    ...
    for (let i = rowStart + 1; i < lines.length; i++) {
        ...
        if (line.trim() && indent <= rowIndent && /^-/.test(line.trimStart()))
            break;
        ...
        const style = m[3] ?? '';
        if (!style)
            return { rowStart, keyIndent, key, style: '', contentStart: i, contentEnd: i + 1 };
        let end = i + 1;
...
export function readPersonaText(text) {
...
async function readPreset(name) {
    ...
    try {
        ...
        if (nm)
            meta.name = nm[1].trim().replace(/^["']|["']$/g, '');
        if (de)
            meta.description = de[1].trim().replace(/^["']|["']$/g, '');
    }
...
async function listPresets(active) {
    const out = [];
    let names = [];
    try {
 ...
 */
export function replacePersonaText(text, persona) {
    const eol = text.includes('\r\n') ? '\r\n' : '\n';
    const lines = text.split(/\r?\n/);
    const block = findPersonaBlock(lines);
    if (!block)
        throw new Error('该 preset 的 agent.cordis.yml 中找不到 persona 行（id/name 含 persona）');
...
async function writePersona(name, persona) {
    if (!/^[\w.@-]+$/.test(name))
        throw new Error('非法 preset 名（仅允许字母数字与 . @ - _）');
...
async function copyPreset(from, to) {
    if (!/^[\w.@-]+$/.test(to))
        throw new Error('非法 preset 名');
    const src = join(presetsDir(), from);
    const dst = join(presetsDir(), to);
    if (!(await exists(src)))
        throw new Error(`源 preset 不存在：${from}`);
    if (await exists(dst))
        throw new Error(`目标 preset 已存在：${to}`);
            ...
            try {
                ...
                if (path === `${API}/persona` && req.method === 'POST') {
                    ...
                    if (typeof persona !== 'string' || !persona.trim())
                        return sendJson(res, 400, { ok: false, error: 'persona text required' });
                ...
                if (path === `${API}/preset/copy` && req.method === 'POST') {
                    ...
                    if (!from || !to)
                        return sendJson(res, 400, { ok: false, error: 'from/to required' });
...
export function apply(ctx) {
    if (typeof ctx.inject === 'function') {
        ctx.inject(['webServer'], (child) => {
            const webServer = child.webServer ?? child.get?.('webServer');
            if (!webServer)
                return;
    ...
    const direct = ctx.webServer ?? ctx.get?.('webServer');
    if (!direct) {
