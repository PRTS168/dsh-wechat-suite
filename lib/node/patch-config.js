...
export async function applyPatchConfig(file, updates) {
    ...
    function valueForRaw(key) {
        ...
        if (key === 'platforms')
            return (before.values.platforms ?? '').trim() === '' ? null : before.values.platforms;
    ...
    function pushPlatforms(target) {
        const list = platformList();
        if (list && list.length > 0)
            target.push(`${ind}platforms:`, ...list.map((entry) => `${itemInd}- ${yamlStr(entry)}`));
    }
    const scalarLine = (key, v) => {
        const field = CONFIG_FIELDS.find((f) => f.key === key);
    ...
    if (parsed.found) {
        ...
        pushPlatforms(outBlock);
        for (const field of CONFIG_FIELDS) {
            if (field.key === 'allowFrom' || field.key === 'platforms')
                continue;
