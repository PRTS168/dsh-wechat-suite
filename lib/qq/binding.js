...
async function postJson(url, payload, timeoutMs) {
    ...
    try {
        ...
        if (!response.ok)
            throw new Error(`绑定接口 HTTP ${response.status}: ${text.slice(0, 200)}`);
 ...
 */
export function decryptBindSecret(encryptedSecret, bindKey) {
    const key = Buffer.from(bindKey, 'base64');
    const raw = Buffer.from(encryptedSecret, 'base64');
    if (key.byteLength !== 32)
        throw new Error('绑定密钥格式异常：需要 32 字节 AES-256 密钥');
    if (raw.byteLength <= 28)
        throw new Error('凭证密文格式异常：太短');
...
export async function createBindTask(opts = {}) {
    ...
    if (!taskId)
        throw new Error('绑定任务响应缺少 task_id');
    return {
        taskId,
        bindKey,
...
/** 第三步：轮询一次绑定结果。 */
export async function pollBindResult(opts) {
    if (!opts.taskId)
        throw new Error('缺少 task_id');
    if (!opts.bindKey)
        throw new Error('缺少 bind_key');
    const host = hostOf(opts.host);
    const data = await postJson(`https://${host}/lite/poll_bind_result`, { task_id: opts.taskId }, opts.timeoutMs ?? 10_000);
    const payload = (data.data ?? {});
