 ...
 */
function directPostJson(url, body, headers, timeoutMs, signal) {
...
export async function postJson(opts) {
    const { baseUrl = ILINK_BASE_URL, endpoint, payload, token, timeoutMs = API_TIMEOUT_MS, fetchImpl = fetch, signal, } = opts;
    ...
    try {
        if (preferDirectConnection) {
            try {
                ...
                if (error instanceof IlinkError || isAbortError(error))
                    throw error;
...
export async function getUpdates(opts) {
    ...
    try {
        ...
        if (error instanceof DOMException && error.name === 'AbortError') {
            ...
            if (signal?.aborted)
                return { messages: [], syncBuf, raw: { ret: 0, msgs: [] }, cancelled: true };
...
export async function sendFileItemMessage(opts) {
    const { baseUrl, token, to, itemType, encryptQueryParam, aesKeyB64Hex, ciphertextSize, plaintextSize, durationSec, fileName, contextToken, clientId, timeoutMs, fetchImpl, } = opts;
    const media = {
        encrypt_query_param: encryptQueryParam,
        aes_key: aesKeyB64Hex,
        encrypt_type: 1,
