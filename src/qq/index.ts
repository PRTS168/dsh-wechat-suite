...
import { ITEM_TEXT, type GatewayStatus, type InboundMessage } from '../gateway/types.ts'
import { describeFetchError, QqApiClient, type QqSendOutcome } from './client.ts'
import {
  QQ_C2C_PASSIVE_REPLY_LIMIT,
  QQ_ERRCODE,
  QQ_DEFAULT_HEARTBEAT_MS,
...
export class QqGateway extends Service {
  ...
  private api: QqApiClient | undefined
  ...
  private identifyTimer: ReturnType<typeof setTimeout> | undefined
  ...
  private readonly dedupWindowMs = 5 * 60_000
   ...
   * 与微信那份彻底隔离。
   */
  private writeStatus(patch: Record<string, unknown>): void {
    try {
  ...
  async start(): Promise<void> {
    ...
    const readFailure = (key: string) => (error: unknown) => {
  ...
  private async connect(): Promise<void> {
    if (!this.api || this.stopRequested) return
  ...
  private handlePayload(raw: unknown, token: string): void {
    try {
  ...
  private handlePayloadInner(raw: unknown, token: string): void {
    let payload: QqPayload
    try {
      payload = JSON.parse(String(raw)) as QqPayload
    } catch {
    ...
    switch (payload.op) {
      case QQ_OP.HELLO: {
        const interval = (payload.d as QqHello | undefined)?.heartbeat_interval
      ...
      case QQ_OP.INVALID_SESSION: {
        ...
        if (this.supersededCount >= 2) {
          this.ctx.emit('qq/fatal', new Error(
            'QQ 长连刚建立就被判为无效（op 9，连续 2 次）：通常意味着有第二个实例正在用同一个机器人' +
            '（另一个进程、另一台机器，或同一 profile 里挂了两份）。QQ 一个机器人同时只允许一个监听者，' +
            '请停掉多余的那个。已停止重连。',
          ))
          this.setStatus('error')
          this.stopRequested = true
          try { this.socket?.close() } catch { /* 已经关了 */ }
          return
        }
   ...
   */
  private scheduleIdentify(token: string, delayMs = 500): void {
    ...
    if (this.authFailures > MAX_AUTH_FAILURES) {
      this.ctx.emit('qq/fatal', new Error(
        `鉴权连续失败 ${this.authFailures - 1} 次（连接被服务端判为无效），已停止重连：` +
    ...
    this.identifyTimer = setTimeout(() => {
      this.identifyTimer = undefined
      if (!this.stopRequested) this.identify(token)
    }, delay)
    this.identifyTimer.unref?.()
  }

  private send(payload: QqPayload): void {
    const socket = this.socket
  ...
  private handleDispatch(payload: QqPayload): void {
    switch (payload.t) {
      case 'READY': {
        const ready = payload.d as QqReady | undefined
      ...
      case QQ_EVENT_C2C_MESSAGE:
        this.handleC2CMessage(payload.d as QqC2CMessageEvent | undefined)
        return
      default:
  ...
  private pruneState(): void {
    ...
    for (const [id, at] of this.seen) {
      if (at < floor) this.seen.delete(id)
    }
    // 被动回复额度与"最后一条消息"都只对仍在窗口内的消息有意义。
    for (const id of this.replyBudget.keys()) {
      if (!this.seen.has(id)) this.replyBudget.delete(id)
    }
    for (const [sender, id] of this.lastMsgId) {
      if (!this.seen.has(id)) this.lastMsgId.delete(sender)
    }
  }

  private handleC2CMessage(d: QqC2CMessageEvent | undefined): void {
    ...
    if (this.c.allowFrom.length > 0 && !this.c.allowFrom.includes(sender)) {
      ...
      this.report(new Error(
        `忽略了一条非白名单的 QQ 消息：sender=${sender}。若这是你自己，把这个 openid 加进 allowFrom 即可。`,
      ))
      return
    }
    if (messageId) {
   ...
   * 结果（`/problems` 就是为它建的）。
   */
  async sendText(to: string, text: string, clientId?: string): Promise<QqSendOutcome> {
    void clientId
    if (!this.api) return { success: false, error: 'QQ 网关未配置（缺 AppID/AppSecret）' }
    ...
    if (msgId && !passive) {
      this.report(new Error(
        `被动回复额度用尽（${used}/${QQ_C2C_PASSIVE_REPLY_LIMIT}），这条改为主动消息；` +
        'QQ 对主动消息有配额且用户可关闭接收',
      ))
    }
    const outcome = await this.api.sendC2CText(to, text, passive
      ? { msgId, msgSeq: used + 1 }
      : {})
    if (outcome.success && passive && msgId) this.replyBudget.set(msgId, used + 1)
    ...
    if (passiveRejected) {
      if (msgId) this.replyBudget.set(msgId, QQ_C2C_PASSIVE_REPLY_LIMIT)
   ...
   */
  async sendImage(to: string, filePath: string): Promise<QqSendOutcome> {
    void to
    void filePath
    return { success: false, error: 'QQ 网关暂不支持发送图片（阶段③：按官方三步上传流程实现）' }
  }

  async sendFile(to: string, filePath: string, fileName?: string): Promise<QqSendOutcome> {
    void to
    void filePath
    void fileName
   ...
   */
  get capabilities(): { media: boolean; voice: boolean; replyBudgetPerInbound: number } {
    return { media: false, voice: false, replyBudgetPerInbound: QQ_C2C_PASSIVE_REPLY_LIMIT }
  }

  /** 入站媒体下载：阶段③。现在返回 null —— 上层的语义是"拿不到字节"，会如实上报。 */
  async downloadImage(item: unknown): Promise<{ bytes: Uint8Array; mediaType: 'image/png' } | null> {
    void item
    return null
  }

  async downloadVoice(item: unknown): Promise<Uint8Array | null> {
    void item
    return null
  }

  async downloadAttachment(item: unknown): Promise<{ bytes: Uint8Array; fileName?: string } | null> {
    void item
    return null
  }
}
...
async function readCredential(
  ...
  try {
    const credentials = ctx.get('credentials') as { resolve?: (ref: unknown) => Promise<unknown> } | undefined
