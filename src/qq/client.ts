...
export class QqApiClient {
   ...
   */
  private preferDirectConnection = false
  /**
   ...
   */
  private readonly opts: QqApiOptions
  ...
  private async request<T>(path: string, init: RequestInit & { json?: unknown }): Promise<T> {
    ...
    const viaFetch = async (): Promise<string> => {
      ...
      try {
        ...
        if (!response.ok) throw new Error(`QQ API ${path} HTTP ${response.status}: ${text.slice(0, 200)}`)
        return text
      } finally {
        clearTimeout(timer)
      }
    ...
    const viaDirect = async (): Promise<string> => {
      if (body === undefined) throw new Error('direct path needs a body')
  ...
  async accessToken(): Promise<string> {
    ...
    const raw = await this.request<QqAccessTokenResponse>(QQ_TOKEN_PATH, {
      method: 'POST',
      json: { appId: this.opts.appId, clientSecret: this.opts.clientSecret },
    })
    const value = raw.access_token
    if (!value) throw new Error('QQ API 没有返回 access_token（检查 appId / clientSecret）')
  ...
  async gatewayUrl(): Promise<string> {
    const raw = await this.request<QqGatewayResponse>(QQ_GATEWAY_PATH, {
      method: 'GET',
      headers: { authorization: `QQBot ${await this.accessToken()}` },
    })
    if (!raw.url) throw new Error('QQ API 没有返回 WebSocket 接入点')
    return raw.url
  }
  ...
  async sendC2CText(
    ...
    try {
      const raw = await this.request<QqSendMessageResponse>(qqC2CMessagesPath(userOpenid), {
        method: 'POST',
        headers: { authorization: `QQBot ${await this.accessToken()}` },
        json: {
          content,
