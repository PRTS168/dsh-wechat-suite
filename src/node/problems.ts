...
function signature(kind: string, message: string): string {
...
export class ProblemReporter {
  ...
  private readonly notifyWindowMs: number
  private readonly maxNoticesPerHour: number
  private notifyOwner: ((text: string) => void) | undefined
  private logger: ((level: 'warn' | 'error', text: string) => void) | undefined

  private readonly records = new Map<string, ProblemRecord>()
  ...
  report(kind: string, error: unknown, options: ProblemOptions = {}): ProblemRecord {
    ...
    if (options.notify === false) return record
    const notice = this.buildNotice(record)
    if (!notice) return record
  ...
  total(): number {
    let sum = 0
    for (const record of this.records.values()) sum += record.count
    return sum
  }
   ...
   */
  private buildNotice(record: ProblemRecord): string | null {
    const now = this.now().getTime()
    const key = signature(record.kind, record.message)
    const last = this.lastNoticeAt.get(key) ?? 0
    if (now - last < this.notifyWindowMs) return null
    this.noticeTimes = this.noticeTimes.filter((time) => now - time < 60 * 60 * 1000)
    if (this.noticeTimes.length >= this.maxNoticesPerHour) return null
  ...
  private rotateIfNeeded(): void {
    ...
    if (now - this.lastRotateCheck < 60_000) return
    this.lastRotateCheck = now
    try {
      if (!existsSync(this.file)) return
      if (statSync(this.file).size < PROBLEM_LOG_LIMIT_BYTES) return
 ...
 */
export function formatProblems(reporter: ProblemReporter, limit = 8): string {
  const rows = reporter.recent(limit)
  if (rows.length === 0) return '✅ 最近没有记录到问题。'
...
export function problemsSummary(reporter: ProblemReporter, withinMs = 24 * 60 * 60 * 1000): string | null {
