...
export interface ImageAttachmentSaver {
  saveImage(input: { data: Uint8Array; mediaType: NativeImageMediaType; name?: string }): Promise<SavedImageRef>
}
...
export function parseRoute(value: string | undefined): { provider: string; model: string } | undefined {
...
export async function declaresImageInput(
...
export async function buildImageBlock(
  attachments: ImageAttachmentSaver | undefined,
  image: { data: Uint8Array; mediaType: NativeImageMediaType; name?: string },
): Promise<ContentBlock> {
  if (!attachments) throw new Error('image attachment service unavailable')
  const ref = await attachments.saveImage(image)
  return { type: 'image', attachment: ref } as unknown as ContentBlock
}
