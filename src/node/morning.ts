...
export class MorningService {
  ...
  async update(patch: Partial<MorningConfig>): Promise<{ config: MorningConfig; saved: boolean }> {
  ...
  private arm(): void {
    ...
    if (!Number.isFinite(armDelay) || armDelay < 0) {
      this.ctx.logger?.warn?.(
        '[dsh-chatnode-wechat] morning greeting not armed: unusable delay %s (time=%s, now=%s)',
        String(armDelay),
        this.config.time,
        String(now),
      )
