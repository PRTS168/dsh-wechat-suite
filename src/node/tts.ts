...
export async function synthesizeSpeech(
  ...
  if (!cfg.apiKey) throw new Error('synthesizeSpeech: no apiKey configured')
  if (!cfg.voice) throw new Error('synthesizeSpeech: no cloned voice uri configured')
  if (!text.trim()) throw new Error('synthesizeSpeech: text is required')
