 ...
 */
export function ensureMemory(file: string): string {
  if (existsSync(file)) return readMemory(file)
  try {
 ...
 */
export function writeMemory(file: string, text: string): MemoryWriteResult {
  const existing = readMemory(file)
  if (text.length > MEMORY_LIMIT_CHARS && text.length >= existing.length) return { ok: false, reason: 'cap' }
...
export function appendMemoryLog(file: string, line: string): void {
  try {
...
export function rotateIfLarge(file: string, limit: number): void {
  try {
 ...
 */
export function pruneBackups(file: string, keep = 5): void {
  try {
    const dir = dirname(file)
    const prefix = `${basename(file)}.bak-`
    const rows: Array<{ name: string; at: number }> = []
...
/** One fact line with the section it lives in. */
interface FactRef {
  index: number
  section: string
  text: string
 ...
 */
function factIndex(lines: string[]): FactRef[] {
  const refs: FactRef[] = []
  ...
  lines.forEach((line, index) => {
    ...
    if (trimmed.startsWith('## ')) {
      ...
      section = (MEMORY_SECTIONS as readonly string[]).includes(name) ? name : null
      return
    }
    if (!section || !isFactLine(line)) return
 ...
 */
export function applyPatch(file: string, patch: MemoryPatch, origin: string): MemoryApplyResult {
  const result: MemoryApplyResult = { added: 0, updated: 0, expired: 0, skipped: [] }
  ...
  for (const entry of patch.add ?? []) {
    ...
    if (factIndex(lines).some((ref) => ref.section === section && ref.text === value)) continue
  ...
  for (const entry of patch.update ?? []) {
    ...
    for (const ref of matches.slice(1).reverse()) lines.splice(ref.index, 1)
    text = lines.join('\n')
    result.updated += 1
  }
  ...
  for (const raw of patch.expire ?? []) {
    ...
    for (const ref of matches.slice().reverse()) lines.splice(ref.index, 1)
  ...
  if (result.added + result.updated + result.expired > 0) {
    ...
    appendMemoryLog(
      file,
      `+${result.added} ~${result.updated} -${result.expired}（origin=${origin}）` +
 ...
 */
export function memoryBriefing(file: string): string | null {
...
function factBlocks(text: string): string[][] {
  ...
  const flush = (): void => {
    if (heading && facts.length > 0) blocks.push([heading, ...facts])
    heading = null
    facts = []
  }
  ...
  for (const raw of text.split('\n')) {
    ...
    if (line.startsWith('> ') || line.startsWith('# ')) continue // title + the file's own guidance
    if (line.startsWith('## ')) {
      flush()
      heading = line === `## ${MEMORY_EXPIRED_SECTION}` ? null : line
      continue
    }
    if (heading && isFactLine(line)) facts.push(line.trim())
  }
  flush()
  return blocks
}
...
export function extractUtterances(events: readonly unknown[]): MemoryUtterance[] {
  ...
  for (const raw of events) {
    const event = raw as { type?: string; data?: { content?: unknown } }
    if (event?.type !== 'user/message') continue
    const blocks = Array.isArray(event.data?.content) ? (event.data?.content as Array<{ type?: string; text?: string }>) : []
    const text = stripBackground(
      blocks
    ...
    if (body) out.push({ at: last[2] ?? '', text: body })
  }
  return out
}
...
export function consolidationPrompt(digest: string, current: string): string {
  return [
 ...
 */
export function parsePatch(reply: string): MemoryPatch | null {
  const cleaned = reply.replace(/```[a-z]*\n?/gi, '').trim()
  const start = cleaned.indexOf('{')
  const end = cleaned.lastIndexOf('}')
  if (start < 0 || end <= start) return null
  try {
    const parsed = JSON.parse(cleaned.slice(start, end + 1)) as Partial<MemoryPatch>
...
/** Scheduler bookkeeping persisted between runs. */interface MemoryState {
  lastRunAt?: string
  lastMessageAt?: string
  /** `lastMessageAt` as of the last completed run: no advance means no news. */
  lastRunMessageAt?: string
}
...
export class MemoryService {
  ...
  private readonly injectEvery: number
  private readonly minUtterances: number
  ...
  private readonly injected = new Map<string, { count: number; hash: string }>()
  private lastMessageAt = 0
  /** `lastMessageAt` as of the last daily run (0 = never run). */
  private lastRunMessageAt = 0
   ...
   * reporting line.
   */
  async consolidateNow(session: Session | undefined, route: { provider: string; model: string } | undefined, reason = 'manual'): Promise<ConsolidationReport> {
    const report: ConsolidationReport = { ran: false, reason, utterances: 0, added: 0, updated: 0, expired: 0, skipped: 0 }
    if (this.running) return { ...report, reason: `${reason}: already running` }
    if (!session || !route) return { ...report, reason: `${reason}: no session or model route` }
    const llm = this.ctx.get('llm') as { stream(options: unknown): AsyncIterable<unknown> } | undefined
    if (!llm?.stream) return { ...report, reason: `${reason}: llm service unavailable` }
    this.running = true
    try {
      const utterances = extractUtterances(session.snapshotEvents() as unknown[])
  ...
  private async onTimer(): Promise<void> {
    try {
      ...
      if (this.lastMessageAt > 0 && idleFor < this.idleMs) return
  ...
  routeProvider: (() => { provider: string; model: string } | undefined) | undefined
  ...
  private loadState(): void {
    try {
      ...
      if (parsed.lastMessageAt) this.lastMessageAt = Number(new Date(parsed.lastMessageAt)) || 0
      if (parsed.lastRunMessageAt) this.lastRunMessageAt = Number(new Date(parsed.lastRunMessageAt)) || 0
    } catch (error) {
      ...
      if ((error as { code?: string }).code !== 'ENOENT') this.onProblem?.('memory/state', error, `file=${this.stateFile()}`)
    }
  }
