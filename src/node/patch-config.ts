...
import { pruneBackups } from './memory.ts'
...
export async function readPatchFile(
  file: string,
): Promise<{ exists: boolean; parsed: ParsedPatch; patch: PatchValues; unreadable?: string }> {
  try {
    ...
    if (code === 'ENOENT') return { exists: false, parsed, patch: { values: {}, allowFrom: [] } }
    return {
      exists: true,
      parsed,
...
export async function applyPatchConfig(file: string, updates: Record<string, string | null | undefined>): Promise<ApplyResult> {
  for (const [key, value] of Object.entries(updates)) {
    if (typeof value === 'string' && value !== '') validateUpdate(key, value)
  }
  let text = ''
  let exists = true
  try {
  ...
  function pushAllowFrom(target: string[]): void {
