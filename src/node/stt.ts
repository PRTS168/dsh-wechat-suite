...
export function audioExtension(bytes: Uint8Array): string {
  // MP4/M4A: ....ftyp
  if (bytes.length >= 12 && bytes[4] === 0x66 && bytes[5] === 0x74 && bytes[6] === 0x79 && bytes[7] === 0x70) return 'm4a'
  // AMR: "#!AMR"
  if (bytes.length >= 6 && bytes[0] === 0x23 && bytes[1] === 0x21 && bytes[2] === 0x41 && bytes[3] === 0x4d && bytes[4] === 0x52) return 'amr'
  // WAV: RIFF....WAVE
  ...
  if (bytes.length >= 3 && ((bytes[0] === 0x49 && bytes[1] === 0x44 && bytes[2] === 0x33) || (bytes[0] === 0xff && ((bytes[1] ?? 0) & 0xe0) === 0xe0))) return 'mp3'
...
export async function transcribeSpeech(
  ...
  if (!cfg.apiKey) throw new Error('transcribeSpeech: no apiKey configured')
  if (bytes.length === 0) throw new Error('transcribeSpeech: empty audio')
  if (bytes.length > MAX_AUDIO_BYTES) throw new Error(`transcribeSpeech: audio too large (${bytes.length} bytes)`)
  ...
  try {
    ...
    if (!text) throw new Error('SiliconFlow ASR returned empty text')
    return text
  } finally {
    clearTimeout(timer)
  }
