...
export function attachApprovalBridge(node: WechatConversationNode): () => void {
  ...
  // Routed dispatch: the host emits this event through a *scope target* —
  //   ctx.waterfall(scopeTarget(req.agent, req.agent), 'approval/request', req, …)
  // and that filter only admits a listener registered on the agent itself, on
  // one of the agent's ancestors, or on an **untagged** scope. The plugin's own
  // context is none of those, so registering here (which is what this used to
  // do) meant the request never arrived: the WeChat user saw nothing while the
  // tool call sat in the approval window until it timed out.
  //
  // The root context is the untagged standing composition, so it receives every
  // routed event; `ownsAgent()` above keeps the answer limited to the agent this
  // bridge actually drives, and everything else is delegated with `next()`.
