...
export async function generateImage(
  ...
  if (!cfg.apiKey) throw new Error('generateImage: no apiKey configured')
  if (!prompt.trim()) throw new Error('generateImage: prompt is required')
  ...
  try {
    ...
    data = await response.json() as { images?: Array<{ url?: string }> }
  } finally {
  ...
  try {
    ...
    if (raw.length === 0 || raw.length > MAX_BYTES) throw new Error(`image download size ${raw.length}`)
    bytes = raw
  } finally {
