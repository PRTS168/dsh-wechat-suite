...
export class WechatGateway extends Service {
   ...
   */
  async downloadVoice(item: WireItem): Promise<Uint8Array | null> {
   ...
   */
  async sendVoice(to: string, filePath: string, durationSec?: number): Promise<SendResult> {
   ...
   * directly in WeChat.
   */
  async sendFile(to: string, filePath: string, fileName?: string): Promise<SendResult> {
    return this.sendMediaAttachment(to, filePath, {
      itemKind: 'file',
  ...
  private async sendMediaAttachment(
    to: string,
    filePath: string,
    opts: { itemKind: 'voice' | 'file'; durationSec?: number; fileName?: string },
  ): Promise<SendResult> {
    if (!this.configured) return { success: false, error: 'not configured' }
    let plaintext: Buffer
    try {
    ...
    // 1. CDN upload ticket.
    let ticket: { uploadParam?: string; uploadFullUrl?: string }
    try {
