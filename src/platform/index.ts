 ...
 */
export function normalizeAllowFromByPlatform(value: unknown): Record<string, string[]> {
  const out: Record<string, string[]> = {}
  if (!value || typeof value !== 'object' || Array.isArray(value)) return out
  for (const [key, list] of Object.entries(value as Record<string, unknown>)) {
    if (!isPlatformId(key) || !Array.isArray(list)) continue
    out[key] = list.filter((entry): entry is string => typeof entry === 'string' && entry.trim().length > 0)
  }
  return out
}
 ...
 */
export function platformNamespace(id: PlatformId): string {
  return `${id}-`
}
 ...
 */
export function resolvePlatformPlan(platforms: unknown, platform: unknown): PlatformPlan {
  const declared: PlatformId = isPlatformId(platform) ? platform : 'wechat'
  const wanted: PlatformId[] = []
  if (Array.isArray(platforms)) {
    for (const value of platforms) {
      if (isPlatformId(value) && !wanted.includes(value)) wanted.push(value)
    }
  }
  if (wanted.length === 0) return { platforms: [declared], primary: declared }
 ...
 */
export function userFence(id: PlatformId): { open: string; close: string } {
  return id === 'qq'
 ...
 */
export const USER_MESSAGE_OPEN_ANY: readonly string[] = PLATFORM_IDS.map((id) => userFence(id).open)
export const USER_MESSAGE_CLOSE_ANY: readonly string[] = PLATFORM_IDS.map((id) => userFence(id).close)
